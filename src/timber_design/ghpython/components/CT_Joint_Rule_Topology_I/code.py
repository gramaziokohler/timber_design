...
class I_TopologyJointRule(Grasshopper.Kernel.GH_ScriptInstance):
    def __init__(self):
        super(I_TopologyJointRule, self).__init__()
        self.classes = {}
        for cls in get_createable_joints():
            supported_topo = cls.SUPPORTED_TOPOLOGY
            if JointTopology.TOPO_I == supported_topo and not issubclass(cls, PlateJoint):
                self.classes[cls.__name__] = cls
        self.joint_type = self.classes.get(ghenv.Component.Params.Output[0].NickName, None)

    def RunScript(self, *args):
        if not self.joint_type:
            ghenv.Component.Message = "Default: ILapJoint"
            ghenv.Component.AddRuntimeMessage(Grasshopper.Kernel.GH_RuntimeMessageLevel.Warning, "ILapJoint is default, change in context menu (right click)")
            return TopologyRule(JointTopology.TOPO_I, ILapJoint)
        ...
        return TopologyRule(JointTopology.TOPO_I, self.joint_type, **kwargs)
